...
class _EmailInboxScreenState extends State<EmailInboxScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final emails = workflow.getFilteredEmails(
      gmailConnected: connectivity.isGmailConnected,
      m365Connected: connectivity.isM365Connected,
    );

    List<EmailItem> filteredEmails = emails.where((email) {
        ...
        children: [
          ...
          Padding(
            ...
            child: SingleChildScrollView(
              ...
              child: Row(
                children: [
                  _buildFilterTab('All', emails.length),
                  const SizedBox(width: 8),
                  _buildFilterTab(
                    'High',
                    emails.where((e) => e.priority == 'High').length,
                  ),
                  const SizedBox(width: 8),
                  _buildFilterTab(
                    'Medium',
                    emails.where((e) => e.priority == 'Medium').length,
                  ),
                  const SizedBox(width: 8),
                  _buildFilterTab(
                    'Low',
                    emails.where((e) => e.priority == 'Low').length,
                  ),
                ],
              ),
            ),
...
class _EmailDetailScreenState extends State<EmailDetailScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final hasGeneratedTask = workflow.tasks.any(
              ...
              child: SizedBox(
                ...
                child: ElevatedButton(
                  onPressed: hasGeneratedTask
                      ? null
                      : () {
                          workflow.createTaskFromEmail(widget.email);
